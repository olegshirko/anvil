...
![boot demo](boot-demo.gif?v=3)

Real session, timings from `/usr/bin/time -p`: `anvil start` restores the
VM from a memory snapshot in 0.8 s (including the CLI bootstrap); the first
`docker run --rm alpine` right after answers in half a second of daemon
time — the rest of what the GIF shows is docker-CLI startup, not the VM.
