...
<p align="center">
  <b>Куй рантаймы контейнеров на macOS и Linux — пока горячо.</b>
</p>
...
Docker Desktop — это как кувалда: тяжёлая, громоздкая и безжалостно садит батарею.  
**Anvil** — это как наковальня для разработчика: построена так, чтобы было удобно, и твой Air не расплавился (но все равно пока тяжелая).

- **Чистая Архитектура** — редкость для инфраструктурного CLI.
- **Закалённое время загрузки** — холодный старт ~20 с.
- **Сплав рантаймов** — Docker, Containerd или Incus.
- **Зеркальный резерв** — когда Docker Hub недоступен, Anvil тянет из из githab.
...
Когда нет докер регистри, Anvil не сдаётся.
